-- Add Better Auth user to public.users and all projects
DO $$
DECLARE
  target_user_id TEXT;
  old_user_id TEXT;
BEGIN
  ...
  SELECT id INTO target_user_id FROM public."user" WHERE email = 'rein_sprites.5y@icloud.com';

  IF target_user_id IS NOT NULL THEN
    -- Check if email exists in users with different ID
    SELECT id INTO old_user_id FROM public.users WHERE email = 'rein_sprites.5y@icloud.com' AND id != target_user_id;
    ...
    VALUES (target_user_id, 'rein_sprites.5y@icloud.com')
